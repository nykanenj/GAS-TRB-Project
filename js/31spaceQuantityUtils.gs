const insertColumnAndFormulas = (sheetObj, newColumnName = 'temp') => {
  ...
  Logger.log('Inserting new space-quantity column to sheet ' + sheetObj.sheet + ' after column number ' + sheetObj.spaceQuantity.lastColumn);
  sheetObj.sheet.insertColumnAfter(sheetObj.spaceQuantity.lastColumn);
  sheetObj.sheet.getRange(sheetObj.startRow, sheetObj.spaceQuantity.lastColumn + 1).setValue(newColumnName);
  
  const newFormula = '=SUM(' +
  sheetObj.sheet.getRange(sheetObj.startRow + 1, sheetObj.headingHashmap[enums.KALUSTESUUNNITELMA.HEADINGS.Maara_tila_1]).getA1Notation() + ':' +
  sheetObj.sheet.getRange(sheetObj.startRow + 1, sheetObj.spaceQuantity.lastColumn + 1).getA1Notation() + ')'; // e.g.'=SUM(K2:M2)';
  const maaraYhtCol = sheetObj.headingHashmap[enums.KALUSTESUUNNITELMA.HEADINGS.Maara_yht];
  const firstFormulaCell = sheetObj.sheet.getRange(sheetObj.startRow + 1, maaraYhtCol); 
  firstFormulaCell.setFormula(newFormula);
  firstFormulaCell.copyTo(sheetObj.sheet.getRange(sheetObj.startRow + 2, maaraYhtCol, sheetObj.nRows - 2)); // Copy Down formula. -2 to omit headingRow and first formula cell
  sheetObj.spaceQuantity.lastColumnIndexRange.setValue(sheetObj.spaceQuantity.lastColumn + 1); //Update config sheet 
};
  
const deleteColumnUpdateFormula = (sheetObj) => {
  
  Logger.log('Deleting last space-quantity column on sheet' + sheetObj.sheet + '. Position ' + sheetObj.spaceQuantity.lastColumn);
  ...
  sheetObj.spaceQuantity.lastColumnIndexRange.setValue(sheetObj.spaceQuantity.lastColumn - 1);
};
...
const repopulateConfigSheet = () => {
  ...
  const KALUSTE_headingRowArray = kalusteSuunnitelmaSheet.getRange(enums.KALUSTESUUNNITELMA.META.STARTROW, 1, 1, KALUSTE_lastColumn).getValues().join().split(',');
  ...
  const ASENNUS_headingRowArray = asennusListaSheet.getRange(enums.ASENNUSLISTA.META.STARTROW, 1, 1, ASENNUS_lastColumn).getValues().join().split(',');
