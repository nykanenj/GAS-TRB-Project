const updateInstallationList = () => {
  ...
  for (let i = 1; i < kalustesuunnitelmaObj.nRows; i++) {
    ...
    for (
      let j = kalustesuunnitelmaObj.spaceQuantity.firstColumn - 1;
      j <= kalustesuunnitelmaObj.spaceQuantity.lastColumn - 1;
      j++
    ) {
      // -1 to get base0 value used in array
      spaceQuantityCells.push(data[i][j]);
    }
    let vahvistettu_toimituspaiva_tehtaalta =
      data[i][colB0[Vahvistettu_toimituspaiva_tehtaalta]];
  ...
  const cellText =
    "Päivitetty: " +
    Utilities.formatDate(now, timeZone, "dd.MM.yyyy 'klo' HH:mm:ss");
